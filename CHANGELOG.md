...
- Implemented `hybridSearch` functionality using BM25 sparse + dense vector
- Created utility script to build filter expression for categories, authors, and date
- Added function to retrieve existing ids to avoid re-embedding the records that already exist in the database.
- Option to get results by `query` (scalar) if only search filters are provided

### Changed

- Modified `ArxivSchema` to include bm25 function, sparse vector, and revised indexes
- Adapted the tests to work with the latest changes
- Changed `serverDest` sync in `logger.ts` to use `true` to resolve complications with `milvus-utils.ts`
- Revised logging calls in `ingestion-service.ts` to be brief, and more meaningful
- Updated `MILVUS_ALIAS` and `MODEL_NAME` in .env.example
